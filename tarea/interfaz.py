...
#agregando el logo 
try:
    logo_img = PhotoImage(file="imagenlogo.png")
    logo_img = logo_img.subsample(13, 16)
except Exception as e:
    logo_img = None

if logo_img:
    logo_label = Label(root, image=logo_img, bg="#e3eaf2")
    logo_label.pack(pady=(10, 0), anchor="n")
...
def ver_datos():
    ...
    conexion.close()
    frame2 = Frame(root2, bg="#f9fafc", bd=2, relief="groove")
    frame2.pack(padx=20, pady=20, fill="both", expand=True)
    for dato in datos:
        label = Label(frame2, text=f"{dato}", font=("Arial", 12), bg="#f9fafc", fg="#222f3e")
...
def eliminar_datos():
    ...
    conexion.close()
    if cantidad == 0:
        messagebox.showwarning("Advertencia", "No hay datos en la base de datos para eliminar.")
        return
...
def actualizar_datos():
    ...
    conexion.close()
    if cantidad == 0:
        messagebox.showwarning("Advertencia", "No hay datos en la base de datos para actualizar.")
        return
    update_win = Toplevel()
...
def buscar_usuario():
    ...
    conexion.close()
    if cantidad == 0:
        messagebox.showwarning("Advertencia", "No hay datos en la base de datos para buscar.")
        return
    buscar_win = Toplevel()
...
ttk.Button(boton_frame, text=" Actualizar", style="Update.TButton",
           command=actualizar_datos).grid(row=1, column=2, padx=10, sticky="ew")
ttk.Button(boton_frame, text=" Eliminar", style="Delete.TButton",
           command=eliminar_datos).grid(row=1, column=3, padx=10, sticky="ew")
ttk.Button(boton_frame, text=" Buscar Usuario", style="Search.TButton",
           command=buscar_usuario).grid(row=1, column=4, padx=10, sticky="ew")
